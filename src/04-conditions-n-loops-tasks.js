...
function getSumBetweenNumbers(n1, n2) {
  const arr = Array.from({ length: n2 - n1 + 1 }, (_, index) => n1 + index);
...
function findFirstSingleChar(str) {
  const result = str.charAt(Array.from(str)
    .findIndex((item, i, arr) => arr.indexOf(item) === arr.lastIndexOf(item)));
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const interArray = [a, b];
  // eslint-disable-next-line no-shadow
  interArray.sort((a, b) => a - b);
  return `${startBracket}${interArray[0]}, ${interArray[1]}${endBracket}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (i % 2 === 1) {
      const newNum = String(Number(arr[i]) * 2).length === 2
...
function getDigitalRoot(num) {
  const sumDigit = String(num).split('').reduce((sum, digit) => sum + Number(digit), 0);
  if (sumDigit > 9) return getDigitalRoot(sumDigit);
  return sumDigit;
}
